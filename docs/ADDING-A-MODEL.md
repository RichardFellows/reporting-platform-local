...
## The SCD2 invariant test

The project ships no singular tests while it is empty. **Recreate this one as
`dbt/tests/scd2_exactly_one_current_version.sql` the moment you convert your
first table to SCD2**, adding an arm per SCD2 model. One test across all of
them rather than one per model: the invariant is a property of the SCD2 shape,
not of any particular table, and a per-model copy is the thing that gets
forgotten when the next table is converted.

It catches two broken states, neither expressible as a column test: MORE than
one open version (the incremental replay failed to close the previous one, so
`as_of()` matches both and doubles every joined row), and ZERO open versions
(the entity vanishes from any point-in-time join after its last `effective_to`,

...
`--clean` suppresses the two injected *failures* while keeping everything else.
It exists so a build can be produced that genuinely passes its tests, which is
what a publish-to-main demonstration needs. Combine with `--version 2` to emit
the clean history as a redelivery of the same COB dates: the prepared layer
takes the latest `_file_version`, so the restatement supersedes the bad data
exactly as a real upstream correction would.

The trade, counterparty and rating feeds are generated by the hand-written
functions below, because their PATHOLOGIES are the point and a generator
working from a feed definition alone cannot produce them.

Every OTHER feed in feeds.yml is generated afterwards by
`reporting_platform.ui.sampledata`, the console's definition-driven generator.
Not a lesser fallback: for a feed whose interest is that it exists and joins
correctly, deriving the columns from the definition is more accurate, and it
means adding a feed does not also mean editing this script. It runs SECOND
because it takes its COB dates and foreign keys from what the other feeds have
already written.
...
# (weight, min_days, max_days). Weights are percentages summing to 100; the
# fallback in _slot_tenor covers a rounding gap rather than a real branch. The
# spread is the point: short trades are remarked daily while a multi-year swap
# is byte-identical for a month at a time, which is what the prepared and
# reporting layers should be asked to cope with.
# See docs/DECISIONS.md#generated-data-must-hold-still
#
# BOTH THIS AND MTM_HOLD WERE LOST, not absent by design -- `_slot_tenor` and
# `gen_trade` referenced them and nothing defined them, so seeding the platform
# raised NameError on the first trade. Restored from 71ad3ba rather than
# re-guessed: these bands decide the maturity profile and mark frequency of the
# generated book.
...
# feeds.yml is generated from its definition -- see the module docstring.
# Listed rather than derived by introspecting `gen_*` names, which would
# silently skip a feed the moment someone renamed one.
...
def main() -> int:
    ...
    # three above -- it was written inline as `sorted(dense)[-N]`, so
    # `--dense-days` under 6 died with `IndexError` from inside the generator
    # rather than being told it had asked for too few days. `--dense-days` is a
    # documented flag, so any value it accepts has to work. (Hoisting also
    # stops re-sorting `dense` once per generated day.)
    ...
    # definition-driven one. Imported HERE rather than at module scope:
    # sampledata resolves the seed root through feeddata.SEED_DIR, bound from
    # REPORTING_SEED_DIR at IMPORT time, so the env var must be set before the
    # import or --out is silently ignored.
    ...
    for feed in derived:
        try:
            ...
            # joins look broken.
            # types= is NOT optional, for the reason ui/app.py's call site
            # spells out: without it sampledata re-guesses from the column
            # NAME, so `haircut_pct` -- declared `decimal` -- gets a value like
            # `HAI-4067`, safe_cast nulls the whole column, and nothing fails.
            # resolve_types is the same map the scaffold uses.
